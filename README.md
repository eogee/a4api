...
| **本地模型** | 把任意 `.gguf` 模型一键变成 OpenAI 兼容的本地/局域网 API 服务：引擎按显卡自动获取、模型库扫描、显存风险评估，一键接入四端配置方案 |
...
- **「接入配置方案」**一键创建指向本地服务的配置方案，到「配置方案」页切换即可让 Claude Code 使用本地模型，与云上 API 无缝互切
...
frontend/          LayUI 前端（js/llama.js 为本地模型页）
